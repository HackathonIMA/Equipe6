...
function martelada(limit, offset) {
    ...
    request({
		url: 'http://api.ima.sp.gov.br/v1/transparencia/projetosAtividades',
		method: 'GET',
		headers: {
        ...
        if(itens instanceof Array) {
            itens.map(function(documents){
				if(documents.id != null) {
					...
					connection.query("INSERT INTO projetos_atividades (id, descricao, funcao, subFuncao, programa, projetoAtividade) VALUES (?, ?, ?, ?, ?, ?);", [documents.id, documents.descricao, documents.funcao, documents.subFuncao, documents.programa, documents.projetoAtividade]);
