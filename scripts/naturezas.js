...
function martelada(limit, offset) {
    ...
    request({
		url: 'http://api.ima.sp.gov.br/v1/transparencia/naturezas',
		method: 'GET',
		headers: {
        ...
        if(itens instanceof Array) {
            itens.map(function(documents){
				if(documents.id != null) {
					...
					connection.query("INSERT INTO naturezas (id, descricao, naturezaReceita) VALUES (?, ?, ?);", [documents.id, documents.descricao, documents.naturezaReceita]);
