...
function martelada(limit, offset) {
    ...
    request({
		url: 'http://api.ima.sp.gov.br/v1/transparencia/programas',
		method: 'GET',
		headers: {
        ...
        if(itens instanceof Array) {
            itens.map(function(documents){
				if(documents.id != null) {
					...
					connection.query("INSERT INTO programas (id, descricao, programa) VALUES (?, ?, ?);", [documents.id, documents.descricao.trim(), documents.programa]);
